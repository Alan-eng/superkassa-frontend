...
const Trigger = ({displayText = 'Please select', color, value, controls, isDropdownOpen, isFocused, ...props}) => {
    
    // Adds a ref to expose the native DOM `.focus()` method when required
    const ref = {
        ref: isFocused ? (button) => { if (isFocused && button) button.focus(); } : null
    };
...
class App extends React.Component {
    ...
    render() {
        const {highlightedIndex, isDropdownOpen, selectedOption, returnFocus} = this.state;
...
const Select1 = () => {
   return(
        <App appData={data} id="id_super_dropdown"/>
   ) 
}
