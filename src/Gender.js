...
class Gender extends React.Component {
    ...
    render() {
        return (
            <Wrapper>
                <Label>Пол</Label>
                <Button onClick={(e) => this.setState({ checked: 'male' })} style={this.state.checked === 'male' ? { backgroundColor: '#B4EAF8' } : {}}>{maleIcon}</Button>
                <Button onClick={(e) => this.setState({ checked: 'female' })} style={this.state.checked === 'female' ? { backgroundColor: '#B4EAF8' } : {}}>{femaleIcon}</Button>
            </Wrapper>
        )
    }
}
